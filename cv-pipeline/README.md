...
The first run downloads the selected Ultralytics weights automatically. The Railway shared worker uses `yolo26s.pt` with 640px overlapping tiles so small, distant people occupy more input pixels. Tiling improves long-view recall but multiplies inference work; free-tier Railway CPU limits mean updates are round-robin rather than perfectly simultaneous. Use a larger model or higher tile rate only with enough CPU/GPU capacity. Very dense or heavily occluded scenes may be better served by a dedicated crowd-counting model such as CSRNet, which can be evaluated later without changing the risk-scoring interface.
