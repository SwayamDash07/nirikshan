...
export default function CitizenMiniMap({ zones, alerts, location }: { zones: Zone[]; alerts: Alert[]; location?: { lat: number; lng: number } }) {
  const [focus, setFocus] = useState<"location" | "campus">("location");
  const campusCenter = useMemo<[number, number]>(() => zones.length ? [zones.reduce((sum, zone) => sum + zone.latitude, 0) / zones.length, zones.reduce((sum, zone) => sum + zone.longitude, 0) / zones.length] : [20.3641, 85.8163], [zones]);
  const center = focus === "location" && location ? [location.lat, location.lng] as [number, number] : campusCenter;
