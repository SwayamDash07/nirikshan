...
function ReportIssue({ session }: { session: Session }) {
  const [venue, setVenue] = useState<Venue>(); const [zones, setZones] = useState<Zone[]>([]); const [zoneId, setZoneId] = useState<number>(); const [description, setDescription] = useState(""); const [loading, setLoading] = useState(true); const [sending, setSending] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  const load = useCallback(async () => { setLoading(true); setError(""); try { const venues = await api<Venue[]>("/api/venues"); if (!venues.length) throw new Error("No venue is available yet."); const venueZones = await api<Zone[]>(`/api/venues/${venues[0].id}/zones`); setVenue(venues[0]); setZones(venueZones); setZoneId((current) => current || venueZones[0]?.id); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not load reporting areas"); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
