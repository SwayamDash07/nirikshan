...
export default function CheckInManagement() {
  ...
  return <Card className={styles.card}>
    ...
    {loading ? <Spinner label="Loading staff check-ins" /> : items.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Staff member</th><th>Status</th><th>Triggered at</th><th>Responded at</th></tr></thead><tbody>{items.map((item) => <tr className={item.status === "pending" ? styles.pending : ""} key={item.id}><td>{item.staffName}</td><td><span className={`${styles.status} ${item.status === "pending" ? styles.statusPending : styles.statusConfirmed}`}>{item.status === "pending" ? "Pending" : "Confirmed"}</span></td><td>{formatTime(item.triggeredAt)}</td><td>{formatTime(item.respondedAt)}</td></tr>)}</tbody></table></div> : <div className={styles.empty}><strong>No check-ins have been triggered</strong><span>Use Trigger Check-In to request a safety confirmation from active staff.</span></div>}
  </Card>;
}
